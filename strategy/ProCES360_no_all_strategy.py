...
class ProCES360_No_All(Strategy):
    ...
    def decode_action(self, action_idx, base_station):
        ...
        if action_idx < edge_server_count:
            ...
            # 转发到相邻基站，不允许转发，随机卸载到边缘服务器
            return 'offload', random.choice(base_station.edge_servers)
        else:
            # 丢弃任务，不允许丢弃，随机卸载到边缘服务器
            return 'offload', random.choice(base_station.edge_servers)
    ...
    def decide(self, base_station, time_slot):
        ...
        if self.pre_state is not None and self.pre_action_idx is not None:
            if self.pre_state[-1] == 0:
                if self.pre_action_idx == self.action_dim - 1:
                    reward = 100
                else:
                    reward = 0
            else:
                if self.pre_action_idx == self.action_dim - 1:
                    ...
                    reward = 0
            # 存储旧策略概率
            self.store_transition(self.pre_state, self.pre_action_idx, self.pre_old_prob, reward, state, False)
    ...
    def post_handle_per_slot(self, base_station, time_slot, result_per_slot):
        ...
        # video_quality_reward = 0
        #
        # reward = transmit_reward + compute_reward + energy_reward + video_quality_reward

        # # 存储最后一个transition，包含旧策略概率
        # a = 1.0 / (result_per_slot.transmit_time * GAMMA1 + 1e-9)  # 越小越好
        # b = 1.0 / (result_per_slot.compute_time * GAMMA2 + 1e-9)
        # c = 1.0 / (result_per_slot.consumed_energy * GAMMA3 + 1e-9)
        # reward = a + b + c
